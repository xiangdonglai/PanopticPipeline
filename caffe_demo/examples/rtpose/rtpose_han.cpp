...
/*
bool extractRawImgs(Mat& image_uchar)
{
	//g_frameIdx,g_panelIdx,g_camIdx should be initialized somewhere
	//I made these variables as global to run this for loop
	for(;g_frameIdx<=g_endIdx;++g_frameIdx)
	{
		int f =g_frameIdx; 
		char save_subFolder[512];
		sprintf(save_subFolder,"%s/%03dXX",FLAGS_write_json.c_str(),int(f/100));
		mkdir(save_subFolder,755);
		sprintf(save_subFolder,"%s/%03dXX/%08d",FLAGS_write_json.c_str(),int(f/100),f);
		mkdir(save_subFolder,755);
		for(  ; g_panelIdx<=g_panelEnd ; ++g_panelIdx)
		{
			int p = g_panelIdx;
			for(; g_camIdx<=24;++g_camIdx)
			{
				int c = g_camIdx;
				if(g_bValidCamChecker[p-1][c-1] ==false)
					continue;
				...
				if(p==0)
					sprintf(filename,"%s/%03dXX/%08d/hd%08d_%02d_%02d.png",FLAGS_write_json.c_str(),int(f/100),f,f,p,c);
				else
					sprintf(filename,"%s/%03dXX/%08d/%08d_%02d_%02d.png",folderName,int(f/100),f,f,p,c);
				printf("\n# input: %s\n",filename);
				...
				//if(out_option == PEAK)
				{
					...
					string rawname = pp.stem().string();
					sprintf(saveFileName,"%s/%s.txt",save_subFolder,rawname.c_str());
				...
				image_uchar = cv::imread(filename.c_str(), CV_LOAD_IMAGE_COLOR);
				if(!image_uchar.data)
					continue;
				else
				{
					g_camIdx++;	//for the next function call
					return true;
				}
			}
		}
...
int g_frameIdx,g_panelIdx;//g_camIdx;
int g_frameStartIdx,g_frameEndIdx,g_panelEnd;
int g_prevPanelIdx =-1;
VGAPanelInfo g_pInfo;
int g_firstTC;
bool extractRawImgs(vector<Mat>& imgVect, vector<int>& camIdxVect)
{
	...
	for( ;g_panelIdx <=g_panelEnd;++g_panelIdx)
	{
		...
		if(g_prevPanelIdx!=g_panelIdx)
		{
			...
			sprintf(rawFileName,"%s/vga/ve%02d/%s",FLAGS_rawDir.c_str(),p,g_rawName.c_str());
		...
		for( ; g_frameIdx<=g_frameEndIdx;++g_frameIdx)
		{
			...
			/*else
				sprintf(save_subFolder,"%s/%03dXX/%08d",save_folder,int(f/100),f);*/
			...
			for(int c=1;c<=24;++c)
			{
				...
				{
					char filename[512];
					//sprintf(filename,"%08d_%02d_%02d",f,p,c);
			
					sprintf(filename,"%s/%03dXX/%08d/%08d_%02d_%02d.txt",FLAGS_write_txt.c_str(),int(f/100),f,f,p,c);
//					printf("check: %s\n",filename);

					//check file existence
					if(exist(filename) )
					{
                                                if(g_frameIdx%100==0 && c==1)
						    printf("# File already exists: %s\n",filename);
						continue;
					}
				}
			...
			for(int cc=0;cc<camIdxVect.size();++cc)
			{
				...
				//Debug
				if(p==0)
					sprintf(filename,"%s/hd%08d_%02d_%02d.png",save_subFolder,f,p,camIdx);
				else
					sprintf(filename,"%s/%08d_%02d_%02d.png",save_subFolder,f,p,camIdx);
				printf("# save images to: %s\n",filename);
				imwrite(filename,imgVect[cc]);
			}*/
		}
        ...
        while(1) 
	{
		...
		for(int imgIdx=0;imgIdx<imgVect.size();++imgIdx)
		{
			...
			Mat image_uchar = imgVect[imgIdx];
	                frame_counter = g_frameIdx-1;//new_frameidx;
        	        global.uistate.current_frame = g_frameIdx-1;//new_frameidx;
  ...
  float FPS = 0;
	...
	while(1) {
		...
		if (!FLAGS_write_txt.empty()) {
			...
			//double a = get_wall_time();
			char fname[256];
			sprintf(fname, "%s/%08d_%02d_%02d.txt", save_subFolder, f.video_frame_number, f.panelIdx,f.camIdx);

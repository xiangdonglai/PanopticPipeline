...
#include "caffe/util/FrameExtractUtil.h"


#include "modeldesc.h"
#include "caffe/util/SyncManager.h"
...
DEFINE_int32(panel_end, 1,
						 "Panel end index");*/
DEFINE_int32(cam_start, 0,
						 "Panel start index");
DEFINE_int32(cam_end, 0,
						 "Panel end index");						 
DEFINE_int32(frame_start, 1,
						 "Panel start index");
DEFINE_int32(frame_end, 1,
						 "Panel end index");
...
int g_camNum=480;
vector< vector<bool> > g_bValidCamChecker(20);
bool OpenRawFiles()
{
	...
	sprintf(camSamplingInfo,"%s/vga_camNames_%03d.txt",cameraSamplingInfoFolder,g_camNum);
	printf("## CameraSamplingInfoFile: %s\n",camSamplingInfo);
	ifstream fin(camSamplingInfo);

	for(int p=0;p<20;++p)
		g_bValidCamChecker[p].resize(24,false);
	...
	else
	{
		...
		{
			fin >> panelIdx >>camIdx;
			g_bValidCamChecker[panelIdx-1][camIdx-1] =true;
		}
	}
	fin.close();
	return true;
...
int g_panelIdx =0;		//HD always 0
int g_frameIdx,g_camStart,g_camEnd;
int g_hdMachineIdx=31;
int g_hdDiskIdx;
int g_hdMachineEnd =46;
int g_prevHdMachineIdx =-1;
int g_prevCamIdx =-1;
int g_frameStartIdx,g_frameEndIdx;//,g_panelEnd;
CameraInfo g_pInfo;
int g_firstTC;

CSyncManager g_syncMan;

SUnivSyncUnitForEachSensor* g_pTargetHDSyncUnit = NULL;

bool extractRawImgs(vector<Mat>& imgVect, vector<int>& camIdxVect)
{
	camIdxVect.clear();
    //g_hdDiskIdx = 1;
    for( ;g_hdMachineIdx <=g_hdMachineEnd;++g_hdMachineIdx)
	{
		if(g_hdMachineIdx !=g_prevHdMachineIdx)
		{
			g_hdDiskIdx = 1;
		}
	for( ; g_hdDiskIdx <=2;++g_hdDiskIdx )
	{
            int hdCamIdx = 2*(g_hdMachineIdx-31) + g_hdDiskIdx -1;
            if(hdCamIdx<g_camStart)
            {
                continue;
            }

            if(hdCamIdx>g_camEnd)
                return false;

		
		//Load New File and Table
		if(g_prevCamIdx!=hdCamIdx)
		{
			if(g_pInfo.m_fp)
			{
				fclose(g_pInfo.m_fp);
			...
			sprintf(rawFileName,"%s/hd/ve%02d-%d/%s.hdraw",FLAGS_rawDir.c_str(),g_hdMachineIdx,g_hdDiskIdx,g_rawName.c_str());
			printf("rawFileName: %s\n",rawFileName);
		
			g_pInfo.m_fp = fopen(rawFileName,"r");
			if(g_pInfo.m_fp==NULL)
			{	
				printf("Failed in opening %s\n",rawFileName);
				continue;
			}
			printf("success: open raw file\n");
			g_prevCamIdx = hdCamIdx;
			g_prevHdMachineIdx = g_hdMachineIdx;
			g_frameIdx = g_frameStartIdx;


			printf("Trying to load syncdata\n");
			char syncFileFolder[521];
			sprintf(syncFileFolder,"%s/sync/syncTables",FLAGS_rawDir.c_str());
			
			CSyncManager::LoadMode loadModeOption = CSyncManager::HDSingleRaw_30;
			CSyncManager syncMan;
			syncMan.loadSelectedTables(syncFileFolder,loadModeOption,g_hdMachineIdx,g_hdDiskIdx);
			g_syncMan = syncMan;		//Just copy should be fine..all the memeber variables are vectors.
			
			g_pTargetHDSyncUnit = g_syncMan.GetFirstHDSyncSyncUnit();
			if(g_pTargetHDSyncUnit==NULL)
			{
				printf("## ERROR:: HD sync table is not valid\n");
				return false;
			}
			//g_firstTC = ConvFrameToTC_vga(g_pInfo.m_fp,0);
			//printf("g_firstTC: %d\n",g_firstTC);
		}

		for( ; g_frameIdx<=g_frameEndIdx;++g_frameIdx)
		{
			...
			sprintf(save_subFolder,"%s/%03dXX/%08d",FLAGS_write_txt.c_str(),int(g_frameIdx/100),g_frameIdx);
			sprintf(fname, "%s/hd%08d_%02d_%02d.txt", save_subFolder, g_frameIdx, 0,hdCamIdx);
			//printf("Check: %s\n",fname);
			if(exist(fname))
			{
				if(g_frameIdx%100==0)
			...
			//Find closest frame
			double expectedUnivTime = g_syncMan.m_hdTCTimeTable_for30fps[g_frameIdx].second;
			double univTimeDiff;
			int frameIdxInRaw = CSyncManager::findClosetFrameIdx_fromUnivTime(expectedUnivTime,*g_pTargetHDSyncUnit,univTimeDiff);
			//printf("frameIdx: %d,frameIdxInRaw: %d, univTimeDiff: %f\n",g_frameIdx,frameIdxInRaw, univTimeDiff);
			bool bValid;
			if(abs(univTimeDiff) <1e-3)     //should be zero
			{
				Mat hdImg;
				bValid = imageExtraction_hd(g_pInfo,frameIdxInRaw,hdImg);
				imgVect.push_back(hdImg);
				camIdxVect.push_back(hdCamIdx);
			}
			else
			{
				printf("FrameDrop detected: frame %d of %d-%d (%d_%d):: univTimeDiff: %f\n",g_frameIdx,g_hdMachineIdx,g_hdDiskIdx,0,hdCamIdx,univTimeDiff);
				//Mat blackImg= Mat::zeros(1080, 1920, CV_8UC3) ;
				//imwrite(outputFileName,blackImg);
				continue;
			}
//			printf("ve%02d-%d: %d/%d=%0.4f%%\n",g_hdMachineIdx,g_hdDiskIdx,cnt,loadNum,float(cnt)/loadNum*100.0);
			
			
			//bool bValid = imageExtraction_vga_withDropChecker(g_pInfo,f,g_firstTC,false,camIdxVect,imgVect);
			if(bValid ==false)
			{
			...
			g_frameIdx++;	//for next loading... Should do -1 to save "currently" extracte data
			return true;
		}
	}
	}			
...
//	double last_frame_time = -1;
	while(1) 
	{
		...
		if(extractRawImgs(imgVect,camIdxVect)==false)
		{
			printf("no more images....\n");
			//No more images to process
			usleep(250*1000.0);
			g_noMoreImageFlag =true;
			continue;
		}

//		printf("check: %d vs %d\n",imgVect.size(), camIdxVect.size());	


		for(int imgIdx=0;imgIdx<imgVect.size();++imgIdx)
		{
			//printf("imgIdx: %d\n",imgIdx);
			Mat image_uchar = imgVect[imgIdx];
            frame_counter = g_frameIdx-1;//-1 is necessary since g_frameIdx is pointing next frame
   	        global.uistate.current_frame = g_frameIdx-1;//-1 is necessary since g_frameIdx is pointing next frame
			
			/*char filename[512];
			if(p==0)
				sprintf(filename,"%08d_%02d_%02d",f,p,camIdx);
			else
				sprintf(filename,"%08d_%02d_%02d",f,p,camIdx);
			printf("fileName: %s\n",filename);*/
...
/*
		...
		//waitKey(10); //120 //400
			...
			f.panelIdx = g_panelIdx;

			f.camIdx = camIdxVect[imgIdx];
			//f.camIdx = imgIdx+1;
...
//		printf("Current image number in queue: %d\n",global.input_queue.size());
...
void* displayFrame(void *i) { //single thread
  ...
  float FPS = 0;
	...
	while(1) {
		...
		//printf("Process: output_queue_ordered: %d\n",global.output_queue_ordered.size());
		/*cout  << "\t input_queue: " << global.input_queue.size()
	           << " | output_queue: " << global.output_queue.size()
	           << " | output_queue_ordered: " << global.output_queue_ordered.size()
	           << " | output_queue_mated: " << global.output_queue_mated.size() <<endl;*/
		...
		if (!FLAGS_write_frames.empty()) {
			...
			char save_subFolder[512];
			sprintf(save_subFolder,"%s/%03dXX",FLAGS_write_frames.c_str(),int(f.video_frame_number/100));
			//printf("save_subFolder: %s\n",save_subFolder);
			mkdir(save_subFolder,755);
			sprintf(save_subFolder,"%s/%03dXX/%08d",FLAGS_write_frames.c_str(),int(f.video_frame_number/100),f.video_frame_number);

			//printf("save_subFolder: %s\n",save_subFolder);
			mkdir(save_subFolder,755);

			char fname[256];
			sprintf(fname, "%s/%08d_%02d_%02d.jpg", save_subFolder, f.video_frame_number, f.panelIdx, f.camIdx);
		...
		if (!FLAGS_write_txt.empty()) 
                {

			
			char save_subFolder[512];
			sprintf(save_subFolder,"%s/%03dXX",FLAGS_write_txt.c_str(),int(f.video_frame_number/100));
			//printf("save_subFolder: %s\n",save_subFolder);
			mkdir(save_subFolder,755);
			sprintf(save_subFolder,"%s/%03dXX/%08d",FLAGS_write_txt.c_str(),int(f.video_frame_number/100),f.video_frame_number);
			//printf("save_subFolder: %s\n",save_subFolder);
			mkdir(save_subFolder,755);
			...
			//double a = get_wall_time();
			char fname[256];
			sprintf(fname, "%s/hd%08d_%02d_%02d.txt", save_subFolder, f.video_frame_number, f.panelIdx,f.camIdx);
			std::ofstream fout(fname,ios::binary);
			fout.write((char*)&f.numPeople,sizeof(int));
            //printf("numPeople: %d\n",f.numPeople);

			for (int ip=0;ip<f.numPeople;ip++) {
				for (int ij=0;ij<num_parts;ij++) 
                                {
					f.joints[ip*num_parts*3 + ij*3+0] *= 1920.0/1280.0; 
					f.joints[ip*num_parts*3 + ij*3+1] *= 1080.0/720.0;
				}
			}

			fout.write((char*)f.joints.get(),sizeof(float)*f.numPeople*num_parts*3);
			fout.close();
/*			fs << "{\n";
			fs << "\"version\":0.1,\n";
			fs << "\"bodies\":[\n";
...
int main(int argc, char *argv[]) {
	...
	g_frameStartIdx = g_frameIdx= FLAGS_frame_start;
	g_frameEndIdx = FLAGS_frame_end;
	//g_panelIdx= FLAGS_panel_start;
	//g_panelEnd= FLAGS_panel_end;
	g_camStart= FLAGS_cam_start;
	g_camEnd= FLAGS_cam_end;
	//g_camIdx= 1;
//	g_camNum =FLAGS_cam_sampleNum;
//	printf("g_camNum: %d\n",g_camNum);

	if (!FLAGS_write_txt.empty()) 
	{
		char save_subFolder[512];
		sprintf(save_subFolder,"%s",FLAGS_write_txt.c_str());
		mkdir(save_subFolder,755);
	}
	if(!FLAGS_rawDir.empty())
	{
		boost::filesystem::path pp(FLAGS_rawDir);
		g_rawName = pp.stem().string();
		printf("g_rawName: %s\n",g_rawName.c_str());

	}

//	if(OpenRawFiles()==false)
//		return -1;

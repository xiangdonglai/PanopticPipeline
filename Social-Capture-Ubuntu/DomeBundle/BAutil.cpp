...
bool loadNVMfast(const string filepath, NVM &nvmdata, const int nMinViews)
{
	...
	cout << "Start to load " << filepath << endl;
...
bool initCameraData(const NVM &nvmdata, const string filepath, vector<CameraData> &camera)
{
	...
	for(int p=0;p<=50;++p)
	{
		...
		for(int c=startCamIdx;c<=endCamIdx;++c)
		{
			...
			opt_intrinsic.push_back(BA_OPT_INTRINSIC_SKEW_ZERO_FIXED);		//default
			opt_lensdistortion.push_back(BA_OPT_LENSDIST_RADIAL_AND_TANGENTIAL); //default
...
bool saveCameraAllParamsSeperately(const string savefolderParm, const string sep, const vector<CameraData> &camera)
{
	string savefolder = savefolderParm + "/calibFiles";
	...
	for (size_t camID = 0; camID < nCameras; camID++)
	{
		...
		string fileName = cam->filename;
		std::size_t n = fileName.rfind(".");
		fileName = fileName.substr(0,n);
				
		if (!cam->available)
		{
			printf("Not available cam: %d\n",cam->filename.c_str());
			/*string intFileName = savefolder + "/" + fileName +".txt";
			ofstream fout_int(intFileName,ios::trunc);
			fout_int.close();*/
			continue;
		}
		...
		string extFileName = savefolder + "/" + fileName +"_ext.txt";
		ofstream fout_ext(extFileName,ios::trunc);
...
//This parameter can be used with ideal (undistorted) images
bool saveCameraAllParamsSeperately_withoutDistortion(const string savefolderParm, const string sep, const vector<CameraData> &camera)
{
	string savefolder = savefolderParm + "/calibFiles_withoutDistortion";
	// CreateDirectory(savefolder.c_str(),NULL);
	boost::filesystem::create_directory(boost::filesystem::path(savefolder));
	size_t nCameras      = camera.size();
	for (size_t camID = 0; camID < nCameras; camID++)
	{
		...
		fout_int << fx << sep << s << sep << u0 << sep
				 << "0" << sep << fy << sep << v0 << sep
				 << "0" << sep << "0" << sep << "1" << sep
				 << 0 << sep << 0 << sep << 0 << sep			//ignore distortion parameters
				 << 0 << sep << 0;				//ignore distortion parameters
...
bool saveReprojectionError(const string filename, const string sep, const residualData &res, const vector<CameraData> &camera, const int order)
{
	...
	ofs	<< "x_obs" << sep << "y_obs" << sep
			<< "x_rep" << sep << "y_rep" << sep
			<< "repError_x" << sep << "repError_y" << endl;
...
bool saveReprojectionErrorByCams(const string filename, const string sep, const residualData &res, const vector<CameraData> &camera, const int order)
{
	ofstream ofs(filename);
	ofs << scientific << setprecision(5);
	if (ofs.fail())
	...
	double reproErrorAll=0.0;
	vector<double> reproErrorByCam;
	reproErrorByCam.resize(camera.size(),0.0);
	vector<int> numVector;
	numVector.resize(camera.size(),0);
	...
	for (int i = 0; i < numdata; i++)
	{
		...
		double res_x = res.error[i][0];
		double res_y = res.error[i][1];
		double res = sqrt( pow(res_x,2.0) + pow(res_y,2.0) );

		reproErrorByCam[camID] += res;
		reproErrorAll+=res;
		numVector[camID] +=1;
	}
	reproErrorAll/=numdata;
	for(int camID=0;camID<reproErrorByCam.size();++camID)
		reproErrorByCam[camID] /= numVector[camID];

	vector< pair< double, int> > sortedRepError;
	sortedRepError.reserve(reproErrorByCam.size());
	for(int camID=0;camID<reproErrorByCam.size();++camID)
		sortedRepError.push_back( pair<double,int>(reproErrorByCam[camID],camID));
	sort(sortedRepError.begin(),sortedRepError.end());
	
	ofs <<"Reprojection error: "<< reproErrorAll <<"\n";
	ofs <<"Number of projections: " << numdata <<"\n";
	ofs <<"\n";
	
	ofs << "cameraName"  << sep << "reproError" << sep << "numPts" << endl;
	for(int i=0;i<sortedRepError.size();++i)
	{	
		int camID = sortedRepError[i].second;
		string cameraname = camera[camID].filename;
		double error = sortedRepError[i].first;
		ofs << cameraname << sep << error << sep << numVector[camID] << endl;
	}
...
bool saveAllData(const string savefolder, const vector<CameraData> &camera, const vector<PointData> &pointdata, const residualData &res, const string prefix, const bool After)
{
	...
	saveCameraAllParamsSeperately(savefolder,sep,camera);
	saveCameraAllParamsSeperately_withoutDistortion(savefolder,sep,camera);
	
	filename = savefolder + "/" + prefix + "3Dpoints" + postfix + ext;
	save3Dpoints(filename, sep, pointdata);


	//filename = savefolder + "/" + prefix + "ReprojectionError" + postfix + ext;		//Too big. Not useful log
	//saveReprojectionError(filename, sep, res, camera, 0);

	filename = savefolder + "/" + prefix + "ReprojectionError_byCams" + postfix + ext;
	saveReprojectionErrorByCams(filename, sep, res, camera, 0);

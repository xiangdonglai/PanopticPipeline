...
#include "caffe/layers/cpm_st_data_layer.hpp"
...
CPMSTDataLayer<Dtype>::CPMSTDataLayer(const LayerParameter& param)
  : BasePrefetchingDataLayer<Dtype>(param),
    reader_(param),
    cpm_st_transform_param_(param.cpm_st_transform_param()){
}

template <typename Dtype>
CPMSTDataLayer<Dtype>::~CPMSTDataLayer() {
  this->StopInternalThread();
}

template <typename Dtype>
void CPMSTDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  cpm_st_data_transformer_.reset(
     new CPMSTDataTransformer<Dtype>(cpm_st_transform_param_, this->phase_));
  cpm_st_data_transformer_->InitRand();
  ...
  // image
  const int crop_size = this->layer_param_.cpm_st_transform_param().crop_size();
  ...
  if (crop_size > 0) {
    ...
    const int height = this->phase_ != TRAIN ? datum.height() :
      this->layer_param_.cpm_st_transform_param().crop_size_y();
    const int width = this->phase_ != TRAIN ? datum.width() :
      this->layer_param_.cpm_st_transform_param().crop_size_x();
  ...
  if (this->output_labels_) {
    const int stride = this->layer_param_.cpm_st_transform_param().stride();
    const int height = this->phase_ != TRAIN ? datum.height() :
      this->layer_param_.cpm_st_transform_param().crop_size_y();
    const int width = this->phase_ != TRAIN ? datum.width() :
      this->layer_param_.cpm_st_transform_param().crop_size_x();

    int num_parts = this->layer_param_.cpm_st_transform_param().num_parts();
...
template<typename Dtype>
void CPMSTDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (datum.encoded()) {
      this->cpm_st_data_transformer_->Transform(cv_img, &(this->transformed_data_));
    } else {
      this->cpm_st_data_transformer_->Transform_nv(datum,
...
INSTANTIATE_CLASS(CPMSTDataLayer);
REGISTER_LAYER_CLASS(CPMSTData);

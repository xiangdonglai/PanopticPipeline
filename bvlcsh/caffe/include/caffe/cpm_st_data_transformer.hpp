#ifndef CAFFE_CPM_ST_DATA_TRANSFORMER_HPP
#define CAFFE_CPM_ST_DATA_TRANSFORMER_HPP
...
class CPMSTDataTransformer {
 public:
  explicit CPMSTDataTransformer(const CPMSTTransformationParameter& param, Phase phase);
  virtual ~CPMSTDataTransformer() {}
 ...
 protected:
  ...
  void putGaussianMaps(Dtype* entry, Point2f center, int stride, int grid_x, int grid_y, double isigma[4], double weight=1.0);
  void dumpEverything(Dtype* transformed_data, Dtype* transformed_label, MetaData);

  // Tranformation parameters
  CPMSTTransformationParameter param_;
